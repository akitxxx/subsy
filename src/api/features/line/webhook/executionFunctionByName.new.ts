...
const handleGetSubscriptions = async (subscriptions: SubscriptionEntity[]): Promise<Output> => {
  ...
  const formattedSubscriptions = subscriptions.map((subscription) => {
    const priceFormatted = formatPrice(subscription.price, subscription.currency);
    const cycleFormatted = getCycleMonths(subscription.cycle);
    return `・${subscription.name}\n　・${priceFormatted}/${cycleFormatted}`;
  }).join('\n');
