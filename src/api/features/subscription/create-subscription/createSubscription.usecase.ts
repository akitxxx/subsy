...
import { ConflictError, type InternalServerError, NotFoundError } from '@/api/shared/error/errors';
...
const run =
  ...
  (input: Input): Effect.Effect<Output, NotFoundError | ConflictError | InternalServerError> =>
    Effect.gen(function* () {
      ...
      const existingSubscriptionCount = yield* subscriptionRepository.countByUserIdAndName({ userId: user.id, name: input.name });
      ...
      yield* subscriptionRepository.create({ entity: newSubscription });
